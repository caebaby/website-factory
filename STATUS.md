...
**2026-07-20 podcast depth + publishing-structure revision (Codex branch, public sync in progress):** removed the Resources podcast card's forced equal height and replaced the empty field with a content-driven series module: Alex-host attribution, three repeatable latest-episode rows, the faint real mark, and a direct `Take the Fit Check` path that explains the conversation handoff. Removed the duplicate full-width podcast shelf. Expanded visible directory schema to all seven resources and documented the production sitemap/content model: one directory, unique article URLs, unique episode URLs, latest-three promotion, required metadata, and sitemap/feed updates. Copy workbook regenerated at 69 pages; all pages rendered clean and accessibility audit is 0 high / 0 medium / 0 low. Resources gate: **0 P0 / 0 P1** with only the accepted LED-019 restrained-accent P2. LED-037 logged. Fresh independent verifier returned **SHIP** after 1440px and 500px Tier-B passes, schema validation, and a zero-broken-reference audit.
