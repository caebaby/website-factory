...
Last updated: 2026-08-04 (Anchor's Alex-copy release is public and the verified nine-route static Replit handoff is at `projects/awp/handoff/`. Johnny's AI media map and prelaunch evidence/checklist are now included. Review-mode crawler controls stay locked until production-domain and documented compliance approval. Read the ANCHOR block first, then FINISH LINE). **2026-07-23: branches merged — this file now carries the Anchor line, the AGL pilot line (see AGL PILOT SESSION block), and the Codex Anchor iterations in one history.**
...
**Live review:** `build/home-v6.html` · public link `caebaby.github.io/anchor-review/` · current local review server on :8201. Prior builds remain as history: v5 = GLM 5.2 via Hermes; v4 = Codex (visible `CODEX DESIGN` marker + `data-design-builder="codex"` provenance metadata); v3 = earlier clean print.
**2026-07-13 (v6):** Clean-sheet design — "The Chart" concept (nautical cartography meets private-bank editorial). Genuinely different composition from v4/v5, not a reskin. Design differences: centered-split nav (brand center, links flanking), vertical hero selector on right edge (not bottom tabs), chart-line dashed latitude decoration in hero, asymmetric editorial pain spread with left-bordered cards, horizontal trust credential row (6-up, no grid), full-width ICP image cards (no stagger offset), centered fit card with horizontal convergence diagram, outcome cards for services (2-col elevated grid with 3-layer shadows + gold bottom-line hover, not numbered rows), vertical process timeline (not horizontal steps), dark navy left-aligned final CTA (not centered cream), 3-column footer with gold rule. Gate: **0 P0 / 0 P1 / 0 P2** (fully clean — first Anchor build to clear with zero P2s). LED-022 logged (accordion collapse under gate). Public mirror commit: `8a0b5d2`.
**2026-07-13 revision:** Restored the prior approved Why Anchor, firm-origin/coordination story, and Insights modules inside v6's “Chart” language (LED-023). Family hero replaced with a contemporary Houston-style multigenerational family and a restrained self-hosted eight-second loop; the same modern image now carries through to Who We Serve. Business-owner wide casting retained. Gate: **0 P0 / 0 P1 / 0 P2**; desktop/mobile Tier-B passed. Brand typography re-verified against `DESIGN.md`: EB Garamond + Nunito Sans are the approved Phase-1 web substitutes; paid Adobe Garamond Pro + Proxima Nova remain Phase 2 pending licensed files.
**2026-07-16 continuation:** Codex retained GLM 5.2's “Chart” direction and completed the site system. Homepage: credential route motion with four explicit unverified placeholders, Why Anchor hover-line graphics, dedicated audience-page links, radio-driven fit-check auto-advance, and a swipe/arrow vertical-video Insights rail. New connected pages: `about.html`, `team.html`, `resources.html`, `blog-template.html`, `oil-gas-executives.html`, `business-owners.html`, and `high-net-worth-families.html`, sharing `assets/anchor-inner.css/js`. All conversion paths lead to `home-v6.html#fit`; unverified credentials/biography remain labeled placeholders. LED-024 logged for duplicate `.story`/`#story` namespace collision found and fixed. Gate: **0 P0 on all eight pages**; homepage 0/0/0, inner-page gold-fill P2 accepted under LED-019 because Anchor forbids gold background fields.
**2026-07-17 team-review release:** tightened the Resources, article, and podcast typography/spacing; restored purposeful numbering; added a four-step Fit Check footer preview; and completed the SEO/GEO review templates. Published the curated v6 site system to `https://caebaby.github.io/anchor-review/` at mirror commit `c6d8c4d`. All nine routes are `noindex,nofollow` for review. Independent pre-push verification: 0 P0 / 0 P1 on every route; homepage 0 P2, other routes only the accepted LED-019 P2. Public allowlist has 0 broken references and excludes restricted Mixkit media (LED-029). Current copy/client-decision workbook: `projects/awp/copy/Anchor_Wealth_Planning_Current_Copy_Review.docx` (22 rendered pages, 0 accessibility findings).
**2026-07-20 local revision (Codex branch, not published):** reworked About, Team, all three audience pages, Resources, article, and podcast as one calmer Anchor system. About/Team now use restrained identity panels; audience heroes use route-specific approved imagery; the Resources directory now mixes articles, podcasts, checklists, and frameworks by decision topic so podcast links are visible in the primary browse path. Desktop and narrow-viewport Tier-B pass complete. All eight routes: **0 P0 / 0 P1**; Resources/article/podcast 0 P2, inner pages retain only accepted LED-019 restrained-accent P2. LED-030 logged. Awaiting Chris visual approval before verifier-agent release pass and public mirror sync.
**2026-07-20 tight-content revision (Codex branch, not published):** translated the AGL article reference's compact information shape into Anchor's brand. Article now uses a tight navy masthead, 660px reading column, companion-podcast path, and sticky Fit Check rail; Resources and podcast mastheads were tightened to match. Gate: **0 P0 / 0 P1** across all three routes; Resources 0 P2, article/podcast only accepted LED-019 P2. LED-031 logged. Awaiting visual approval.
**2026-07-20 article depth + SEO/GEO revision (Codex branch, not published):** removed the visible “One vest. Four clocks.” map; replaced it with source-backed decision, document, and FAQ content. Removed the angled blue masthead stripe across content routes, tightened all inner-page heroes, and strengthened Fit Check actions. Article now has matched visible `FAQPage` schema, updated BlogPosting metadata, IRS/SEC citations, 1,485 visible words, and no hidden core answers. Gate: **0 P0 / 0 P1** across all eight secondary/content routes; LED-032 logged for the closed-FAQ recurrence. Awaiting visual approval and verifier-agent release pass.
**2026-07-20 brand-mark + conversion revision (Codex branch, not published):** imported the client-supplied 2026 primary logo mark as a large low-opacity article masthead watermark and rebuilt the sticky next-step prompt as a square navy conversion panel. Its Fit Check button now uses light text, dark Anchor gold, directional arrow, and layered shadow. Article gate: **0 P0 / 0 P1 / 0 P2**. Asset provenance recorded in `assets/SOURCES.md`; LED-033 logged. Awaiting visual approval.
**2026-07-20 navigation brand revision (Codex branch, not published):** replaced the placeholder “A” tile and hand-drawn anchor symbols with the trimmed client-supplied primary logo mark across all nine connected routes, including homepage, inner pages, Resources, article, and podcast navigation. Full-site gate: **0 P0 / 0 P1** on every route; homepage, Resources, and article 0 P2, remaining restrained-accent notes already accepted under LED-019. Context handoff is current through commit history, STATUS, LEDGER, and `projects/awp/qa/CONTENT_SEO_GEO.md`.
**2026-07-20 unified secondary-masthead revision (Codex branch, not published):** left the homepage hero untouched and brought About, Team, all three audience pages, Resources, article, and podcast under the compact publication masthead approved on the resource article. The old split-image campaign heroes, About/Team initials, Resources circle, and podcast cover-led header were removed; every secondary route now uses the real Anchor mark as a faint watermark over one quiet navy/dot field. Desktop and narrow-viewport Tier-B passed. All eight routes: **0 P0 / 0 P1**; Resources and article 0 P2, remaining restrained-accent notes accepted under LED-019. LED-034 logged. Awaiting Chris visual approval before verifier-agent release pass and public mirror sync.
**2026-07-20 navigation + podcast client-review release:** standardized all nine routes on one exact navigation contract: 76px bar, 1180px inner width with 24px gutters, the client-supplied mark, identical link order, and one `Take the Fit Check` CTA. Renamed the audio system `Anchor Wealth Podcast`, changed audio actions to `Listen to the podcast`, replaced the Resources feature-card circle with the faint real mark, and aligned podcast metadata, transcript language, schema, homepage eyebrow, article companion path, and resource badges. Factory gate: **0 P0 / 0 P1 on all nine routes**; homepage, Resources, and article 0 P2, remaining notes accepted under LED-019. Client copy workbook rebuilt as a 70-page side-by-side document with editable response boxes and 0 accessibility findings. LED-035/036 logged. Two fresh independent agents returned **SHIP** for the noindex client-review mirror. Published and live at `https://caebaby.github.io/anchor-review/` (mirror commit `ba43380`); real podcast audio, duration, and platform URLs remain required before production indexing.
**2026-07-20 podcast depth + publishing-structure release:** removed the Resources podcast card's forced equal height and replaced the empty field with a content-driven series module: Alex-host attribution, three repeatable latest-episode rows, the faint real mark, and a direct `Take the Fit Check` path that explains the conversation handoff. Removed the duplicate full-width podcast shelf. Expanded visible directory schema to all seven resources and documented the production sitemap/content model: one directory, unique article URLs, unique episode URLs, latest-three promotion, required metadata, and sitemap/feed updates. Copy workbook regenerated at 69 pages; all pages rendered clean and accessibility audit is 0 high / 0 medium / 0 low. Resources gate: **0 P0 / 0 P1** with only the accepted LED-019 restrained-accent P2. LED-037 logged. Fresh independent verifier returned **SHIP** after 1440px and 500px Tier-B passes, schema validation, and a zero-broken-reference audit. Published and live at `https://caebaby.github.io/anchor-review/resources.html` (mirror commit `7e82450`).
**2026-07-20 sitewide Anchor-motif release:** replaced every large decorative ring treatment with the same low-opacity client-supplied Anchor watermark across inner conversion cards, About/Team closes, content mastheads, podcast cover template, article Fit Check rail, homepage coordination map, homepage Fit Check card, and homepage final CTA. Functional circles remain intact: playback and carousel controls, progress/process markers, credential dots, and Alex's portrait placeholder. Fresh verifier returned **SHIP** after a static motif sweep, 1440px/500px rendered checks on representative routes, and the nine-route gate: **0 P0 / 0 P1**, homepage 0 P2, remaining routes only accepted LED-019 P2. LED-038 logged. Published and live at `https://caebaby.github.io/anchor-review/` (mirror commit `a7404db`).
**2026-07-20 full-lockup navigation release:** removed the faint Anchor watermark from the homepage Fit Check card and replaced the reconstructed navigation mark/name treatment with the client-supplied full primary light-background lockup across all nine routes. Navigation now uses one exact light 76px bar, 1180px inner width, 24px mobile gutters, identical 158×50 desktop / 145×46 mobile lockup geometry, and unchanged labels/order/CTA; footer branding remains untouched. The source asset was alpha-trimmed only and independently verified pixel-identical to the supplied artwork inside its visible bounds. After the verifier caught and closed a 1px shared-shell mismatch, the fresh release verdict is **SHIP** at 1440px and 500px. Nine-route gate: **0 P0 / 0 P1**; homepage 0 P2, other routes only accepted LED-019 P2. LED-039 logged. Published and live at `https://caebaby.github.io/anchor-review/` (mirror commit `658f772`).
**2026-07-20 client handoff sync:** re-verified the public review as one nine-page site: homepage, About, Team, all three Who We Serve pages, Resources, article, and podcast. Every live route and the public index are byte-for-byte matches to the approved source. Regenerated `projects/awp/copy/Anchor_Copy_Review_Side_by_Side.docx` directly from those nine current HTML pages. Coverage audit checked 581 page-specific content/metadata/interface items plus the shared navigation, footer, legal, and Fit Check blocks with **0 missing items**. The 69-page workbook rendered cleanly, all pages were visually reviewed, and the accessibility audit returned **0 high / 0 medium / 0 low** findings. Public review remains `https://caebaby.github.io/anchor-review/` at mirror commit `658f772`.
**2026-07-20 Resources watermark correction:** removed only the subtle Anchor mark from the Resources directory masthead to avoid repeating the same mark immediately above the Alex-hosted podcast module. The first Pages release retained the old visual in browsers because GitHub cached the unversioned stylesheet for 10 minutes; the route now uses a cache-busted stylesheet URL plus an explicit resource-scoped `content: none` guard. The masthead dot field, all copy, the podcast-module mark, and every other secondary/article/podcast header watermark remain intact. Fresh verifier returned **SHIP** at 1440px and 500px; Resources gate remains **0 P0 / 0 P1** with only accepted LED-019 P2. The rendered public page was rechecked after deployment and is clean. Published live at `https://caebaby.github.io/anchor-review/resources.html` (mirror commit `b345980`).
**2026-07-20 family-hero correction:** restored a young family walking hand-in-hand at sunset across the homepage family hero, HNW card, and both family previews. Replaced the review-only prior cut with licensed Pexels 9361674 footage, removed audio for the ambient loop, and produced a 9:16 derivative from the same footage so all four family members remain visible on phones. The inactive table-scene assets and restricted working cuts have no active homepage references and remain excluded from release. Fresh verifier returned **SHIP** after real playback checks at 1440px and 500px; homepage gate is **0 P0 / 0 P1 / 0 P2**. The deployed desktop/mobile streams were rechecked from the public URL and both play correctly. LED-040 logged. Published live at `https://caebaby.github.io/anchor-review/` (mirror commit `a021ca8`).
**2026-07-20 prioritized copy-workbook revision:** rebuilt `projects/awp/copy/Anchor_Copy_Review_Side_by_Side.docx` as an 18-page client decision workbook instead of a 69-page page-by-page transcription. Part 1 now puts launch-required facts, high-impact positioning, conversion decisions, and content direction first; Part 2 groups only the secondary page copy worth reviewing. Routine navigation/footer labels, repeated buttons, SEO/social/schema metadata, accessibility labels, and duplicated copy were removed from Alex's workload. The current-copy extractor also now preserves spaces around inline HTML. All 18 pages rendered cleanly and the accessibility audit returned **0 high / 0 medium / 0 low** findings. LED-041 logged.
**2026-07-31 hero-media refresh (branch `codex/anchor-hero-media-refresh`, local only):** replaced the overly dark family sunset hero with a brighter, active three-generation family in a modern kitchen (Pexels 8479992), carried the same approved scene into the HNW card and family video previews, and reduced only the family-specific navy scrim so the footage remains legible and bright. Added `build/hero-media-review.html`, which shows the installed family at the real hero crop plus three business-owner desk choices: current paperwork, formal CEO, and smart-casual founder. Business alternatives remain review-only until Chris selects one. Source/licensing records are current in `assets/SOURCES.md`; old sunset assets remain inactive for rollback. Node 25 gate: homepage **0 P0 / 2 pre-existing review-copy P1 / 0 P2**; media-review page **0 P0 / 0 P1 / 1 accepted restrained-accent P2**. Desktop/mobile Tier-B and full-resolution poster checks passed. LED-047 logged. Public GitHub review is intentionally unchanged pending media approval.
**2026-07-31 preferred business-video revision (same branch, local only):** installed Chris's selected Pexels 7552425 Vlada Karpovich boardroom scene as the active business-owner hero, Who We Serve poster, and business preview. Produced separate 1280×720 and 720×1280 self-hosted derivatives; the phone version preserves the full meeting instead of collapsing the scene to one cropped face. The previous paperwork, formal CEO, and smart-casual founder clips remain selectable in `build/hero-media-review.html`; the old paperwork clip remains the explicit Alex-story placeholder only. Source provenance is current. Fresh gate: homepage **0 P0 / 2 pre-existing review-copy P1 / 0 P2**; comparison page **0 P0 / 0 P1 / 1 accepted restrained-accent P2**. Desktop/mobile Tier-B passed. LED-048 logged. Public GitHub review remains unchanged pending final visual approval.
**2026-08-02 hero-media client-review release:** replaced the dark sunset-family hero with the approved bright multigenerational kitchen scene and installed the selected Vlada Karpovich boardroom footage across the business hero, audience card, and preview. The business scene now uses the same 16:9 source at every breakpoint with `object-fit: cover`, so narrow screens crop the outer participants instead of showing artificial blurred bands. Two independent pre-push verifiers returned **SHIP** on the curated nine-file allowlist. Fresh gate: homepage **0 P0 / 2 pre-existing review-copy P1 / 0 P2**; comparison page **0 P0 / 0 P1 / 1 accepted restrained-accent P2**. Desktop and 390px Tier-B passed with no business-video bands. Live HTML and the 874,005-byte deployed video were rechecked after Pages propagation. Published at `https://caebaby.github.io/anchor-review/` (mirror commit `c4b1d3b`).
**2026-08-03 Alex copy-response revision (branch `codex/anchor-hero-media-refresh`, local only):** imported Alex's completed side-by-side workbook and direct audience-order instruction. High-Net-Worth Families now lead the hero, selector, Who We Serve cards, Fit Check, Insights rail, footer specialties, product definition, SEO description, and review materials; Oil & Gas Executives are second and Business Owners third. Replaced credential placeholders with client-supplied 19 years, APMA®, and CRPC®; removed the empty accolade; added Haley Johnson, Dawn Cooper, and Derek Morris with supplied roles; updated Alex's biography, team promise, Why Anchor pillars, family pain statement, and inclusive fit language. Every client-supplied title, biography, experience, and credential claim has an adjacent compliance-pending qualifier. The legal entity/RIA disclosure, missing `$5M–$10M` Fit Check band, team biographies/headshots, and final podcast/contact media remain unresolved. All nine routes pass the factory gate with **0 P0**; the regenerated 18-page side-by-side workbook rendered cleanly and returned **0 accessibility findings**. Desktop/mobile Tier-B passed; fresh independent verifier returned **SHIP** after catching and closing the metadata, Insights-order, and point-of-claim qualifier gaps. LED-050 logged. Public GitHub review remains unchanged pending Chris's visual approval.
**2026-08-03 audience-card art-direction revision (same branch, local only):** replaced generic landscape-to-portrait runtime crops in the homepage Who We Serve cards with dedicated 4:5 assets. The family card now keeps the full modern multigenerational group inside the frame; oil and gas uses a centered pumpjack crop; business owners keeps the two lead decision-makers centered. The family portrait is an OpenAI-assisted derivative of the already approved Pexels 8479992 scene and remains pending final client/compliance approval; all provenance is recorded in `assets/SOURCES.md`. Desktop and 390px Tier-B passed; homepage gate remains **0 P0 / 2 pre-existing review-copy P1 / 0 P2**. LED-051 logged. Public GitHub review remains unchanged pending approval.
**2026-08-03 family-hero edge-to-edge correction (same branch, local only):** retired the blurred-fill 9:16 family derivative that reintroduced visible bands on narrow screens. The active mobile source is now a true 720×1280 pan-and-scan derivative from the same approved Pexels 8479992 footage: it travels across the modern multigenerational group, fills the viewport without letterboxing, and loops back to its starting composition. The desktop family source is unchanged. Actual 390px playback and desktop/mobile frozen-state checks passed; homepage gate remains **0 P0 / 2 pre-existing review-copy P1 / 0 P2**. LED-052 logged. Public GitHub review remains unchanged pending approval.
**2026-08-03 current-client login revision (same branch, local only):** added a quiet but visible `Client Login` link to the desktop navigation, mobile menu, and footer on all nine connected review routes. The destination uses the stable official `https://www.investor360.com/` entry point in a new tab; the supplied Auth0 `/u/login?state=...` URL was not used because its state token is transaction-specific and already failed as a durable destination. Shared nav styling keeps the login distinct without competing with the Fit Check CTA. Verification also closed two responsive-state gaps: inner-page menu selection now resets `aria-expanded`, and the homepage menu now opens throughout its full 721–900px collapsed-nav range. Live open-menu checks at 390px and 820px show zero horizontal overflow and correct post-selection state. All nine routes pass with **0 P0**; LED-053/054 logged. Public GitHub review remains unchanged pending approval.
**2026-08-03 Alex-copy release + Replit handoff:** completed a 21-decision source-to-build audit against Alex's returned workbook. An independent verifier caught four semantic misses that the structural gate could not: the full Why Anchor promise, `Investment strategy` and `Not sure` Fit Check choices, the one-business-day/no-pitch response promise, and the required Fit Check-first CTA hierarchy. All four are fixed; the dated audit is `copy/client-feedback/2026-08-03_ALEX_IMPLEMENTATION_AUDIT.md`. The curated GitHub review is public at `https://caebaby.github.io/anchor-review/` (mirror commit `c38a8b5`). Built `projects/awp/handoff/` as a clean Replit root with `.replit`, static run contract, automated 10-page reference/content check, review-safe `noindex,nofollow` + `robots.txt`, and a production launch checklist. Browser QA passed at 390px, 820px, and 1440px: no horizontal overflow, full-bleed family video, working responsive menus, complete six-choice auto-advance Fit Check, consistent page shells, and zero console warnings/errors. Every route has **0 P0**; homepage retains two declared review-media P1s and secondary routes only the accepted LED-019 restrained-accent P2. Two independent release agents returned **SHIP**. LED-055/056 logged.
**2026-08-04 Johnny AI + compliance-readiness handoff (same branch):** added `handoff/AI-HANDOFF-NOTES.md`, a selector-level nine-item media placement map covering the Alex story, team portraits, Insights clips/posters, podcast cover/audio/transcript/schema, social images, and icons. Rebuilt the Replit checklist around verified evidence and launch ownership; added `PRELAUNCH-VERIFICATION.md` with machine-readable states and a mandatory compliance-approval record; and extended `npm run preflight` to check the media manifest, review crawler controls, local references, all five live external destinations, and the rule that formal approval cannot be marked complete without evidence. Fresh package/browser QA passed all ten files at 390px plus the homepage at 820px/1440px: zero overflow, zero broken images, full-bleed hero video, working mobile menu, Fit Check auto-advance to email, and zero console warnings/errors. Factory gate: **0 P0 on every route**; homepage aliases retain two declared replacement-media P1s and secondary pages only accepted LED-019 P2. Internal FINRA/SEC/MML-MassMutual-oriented preflight is complete; **formal regulatory/firm approval is not received** and production indexing remains blocked until the exact entity/disclosures/links and named approval record are supplied. LED-057/058 logged.
