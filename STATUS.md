...
**2026-07-20 navigation + podcast release candidate (Codex branch, public sync in progress):** standardized all nine routes on one exact navigation contract: 76px bar, 1180px inner width with 24px gutters, the client-supplied mark, identical link order, and one `Take the Fit Check` CTA. Renamed the audio system `Anchor Wealth Podcast`, changed audio actions to `Listen to the podcast`, replaced the Resources feature-card circle with the faint real mark, and aligned podcast metadata, transcript language, schema, homepage eyebrow, article companion path, and resource badges. Factory gate: **0 P0 / 0 P1 on all nine routes**; homepage, Resources, and article 0 P2, remaining notes accepted under LED-019. Client copy workbook rebuilt as a 70-page side-by-side document with editable response boxes and 0 accessibility findings. LED-035/036 logged. Two fresh independent agents returned **SHIP** for the noindex client-review mirror; real podcast audio, duration, and platform URLs remain required before production indexing.
