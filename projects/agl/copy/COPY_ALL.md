...
**Final:** unchanged. [VERIFY: guide name + photo + quote — if the guide is Lane, peer framing only ("an advisor who works with advisors in and after transition"), never "made the transition herself," never recruiter framing.] Booking button stays `href="#" data-verify="booking-url"` per LED-042.
...
**Secondary CTA:** Book a private call — *No pitch, no pipeline. A guide who will tell you the truth, including when the truth is "stay."* [booking URL: `data-verify="booking-url"` per LED-042]
